...
def reset_gp(context, clear_strokes=False):
    ...
    if gp_data.layers.find("srtk_gp_data_layer") < 0:
        gp_data.layers.active = gp_data.layers.new("srtk_gp_data_layer")
    else:
        gp_data.layers.active = gp_data.layers["srtk_gp_data_layer"]
        
    frame0_found = False
    for f in gp_data.layers.active.frames:
        if f.frame_number == 0:
            if clear_strokes:
                f.clear()
            frame0_found = True
...
class BUTTON_OT_draw_grid(Operator):
    ...
    def execute(self, context):
        ...
        verts_prev_stroke = []
        for s in gp_obj.data.layers.active.frames[0].strokes:
